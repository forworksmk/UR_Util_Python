# https://github.com/takuya-ki/onrobot-rg/blob/main/src/onrobot.py
...
class RG():
    ...
    def close_gripper(self, force=400):
        """Closes gripper."""
        self.open_connection()
        params = [force, 0, 16]
        print("Start closing gripper.")
        while True:
            if not self.get_status()[0]:  # not busy
                result = self.client.write_registers(
                    address=0, values=params, unit=65)
                break
        self.close_connection()

    def open_gripper(self, force=400):
        """Opens gripper."""
        self.open_connection()
        params = [force, self.max_width, 16]
    ...
    def move_gripper(self, width, force=400):
        """Moves gripper to the specified width."""
        self.open_connection()
        params = [force, width, 16]
